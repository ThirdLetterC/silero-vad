...
bool wav_reader_open(wav_reader_t* reader, const char* filename) {
    if (reader == nullptr || filename == nullptr) return false;
    ...
    if (header.fmt_size < 16) {
        ...
        if (fread(chunk_id, 1, chunk_id_size, fp) != chunk_id_size) { goto cleanup; }
    }
    ...
    switch (reader->bits_per_sample) {
        case 8: {
            ...
            for (size_t i = 0; i < num_data; ++i) {
                ...
                reader->data[i] = (float)sample / 32'768.0f;
            }
            break;
        }
        case 16: {
...
void wav_writer_init(wav_writer_t* writer, const float* data, size_t num_samples, 
...
bool wav_writer_write(const wav_writer_t* writer, const char* filename) {
    if (writer == nullptr || writer->data == nullptr || filename == nullptr) return false;
    if (writer->num_channel <= 0 || writer->bits_per_sample <= 0 || writer->sample_rate <= 0) return false;
    if ((writer->bits_per_sample % 8) != 0) return false;
    ...
    size_t data_bytes;
    if (ckd_mul(&data_bytes, writer->num_samples, (size_t)writer->num_channel)) goto cleanup;
    if (ckd_mul(&data_bytes, data_bytes, (size_t)(writer->bits_per_sample / 8))) goto cleanup;

    header.data_size = (uint32_t)data_bytes;

    size_t riff_size = sizeof(header) - 8U;
    if (ckd_add(&riff_size, riff_size, (size_t)header.data_size)) goto cleanup;

    size_t bytes_per_second = 0;
    if (ckd_mul(&bytes_per_second, (size_t)writer->sample_rate, (size_t)writer->num_channel)) goto cleanup;
    if (ckd_mul(&bytes_per_second, bytes_per_second, (size_t)(writer->bits_per_sample / 8))) goto cleanup;

    size_t block_size = 0;
    if (ckd_mul(&block_size, (size_t)writer->num_channel, (size_t)(writer->bits_per_sample / 8))) goto cleanup;

    header.size = (uint32_t)riff_size;
    header.bytes_per_second = (uint32_t)bytes_per_second;
    header.block_size = (uint16_t)block_size;

    if (fwrite(&header, 1, sizeof(header), fp) != sizeof(header)) goto cleanup;
